...
/**
 * Checks if a URL is on the same domain as the base URL
 * @param {string} url - The URL to check
...
/**
 * Checks if a URL is on the same domain as the base URL or is an allowed external domain (like feedburner)
 * @param {string} url - The URL to check
 * @param {URL} baseUrl - The base URL for comparison
...
function isAllowedDomain(url, baseUrl) {
	try {
		...
		// Allow feedburner.com as an exception
		if (parsedUrl.hostname === 'feedburner.com' || parsedUrl.hostname.endsWith('.feedburner.com')) {
			return true;
		}
		return false;
	} catch (e) {
...
async function processAnchor(anchor, context) {
	...
	// Only check URLs that are on the same domain or are allowed external domains (like feedburner)
	if (!isAllowedDomain(urlToCheck, baseUrl)) {
		return;
	}
