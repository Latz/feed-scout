...
export default async function blindsearch(instance) {
	...
	while (path.length >= origin.length) {
		...
		endpoints.forEach(endpoint => endpointUrls.push(`${basePath}/${endpoint}`));
	...
	while (i < endpointUrls.length && !(rssFound && atomFound)) {
		...
		instance.emit('log', { module: 'blindsearch', url: url, visited: false });

		try {
			let feedType = await checkFeed(url);

			// Emit log for visited site
			instance.emit('log', { module: 'blindsearch', url: url, visited: true });
			...
			instance.emit('error', { module: 'blindsearch', error: `Error fetching ${url}: ${error.message}` });
		}
		i++;
	}
	
	// Emit end event with totalUrls count
	instance.emit('end', { module: 'blindsearch', feeds, totalUrls: endpointUrls.length });
	return feeds;
}
