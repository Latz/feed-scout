...
|  |  | | |_|_|_____ ___| |_ ___   |   __|___ ___ _| |  |   __|_|___ _| |___ ___ 
|  |  | |  _| |     | .'|  _| -_|  |   __| -_| -_| . |  |   __| |   | . | -_|  _|
|_____|_|_| |_|_|_|_|__,|_| |___|  |__|  |___|___|___|  |__|  |_|_|_|___|___|_|
`;
...
program
  .name(`node uff-cli.js`)
  .description("The ultimate way to find any feed of a site.");
  ...
  if (data.module == "deepSearch") {
    ...
    if (data.module === "blindsearch") {
      ...
      if (data.url === true) {
        ...
        // No need to increment unvisitedCount since it's already set from totalCount
      }

      // Only update display if we have a module from blindsearch
      if (data.url === true || data.url === false) {
...
function showDeepSearchSuggestionIfNeeded() {
  ...
  const wasDeepSearchUsed =
    currentOptions && (currentOptions.deepsearch || currentOptions.all);
  if (!wasDeepSearchUsed && !hasShownDeepSearchSuggestion) {
    ...
    process.stdout.write(
      `${chalk.yellow("Try using the -d or --deepsearch flag to enable it:")}\n`,
    );
