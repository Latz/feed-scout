	...
	if (data.module == 'deepSearch') {
		...
		if (data.module === 'blindsearch') {
			...
			if (data.url === true || data.url === false) {
				...
				const percentage = unvisitedCount > 0 ? Math.round((visitedCount / unvisitedCount) * 100) : 0;
				
				// Create new display showing "Started Blindsearch (visited urls/total urls) percentage%"
				const displayText = `Started Blindsearch (${visitedCount}/${unvisitedCount}) ${percentage}%`;
